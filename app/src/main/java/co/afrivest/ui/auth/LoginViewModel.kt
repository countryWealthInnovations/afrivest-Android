...
class LoginViewModel @Inject constructor(
    ...
    private val _navigateToCurrencySetup = MutableLiveData(false)
    val navigateToCurrencySetup: LiveData<Boolean> = _navigateToCurrencySetup
            ...
            try {
                ...
                when (response) {
                    is Resource.Success -> {
                        ...
                        // Save currency prefs
                        response.data.user.default_currency?.let {
                            preferencesManager.defaultCurrency = it
                        }
                        response.data.user.secondary_currency?.let {
                            preferencesManager.secondaryCurrency = it
                        }
                        preferencesManager.kycBannerHidden = response.data.user.kyc_banner_hidden ?: false

                        // Check email verification
                        if (!response.data.user.email_verified) {
                            _showEmailVerificationDialog.value = true
                        } else if (response.data.user.default_currency == null) {
                            _navigateToCurrencySetup.value = true
                        } else {
