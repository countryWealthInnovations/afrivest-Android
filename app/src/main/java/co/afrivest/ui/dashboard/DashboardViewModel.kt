...
import co.afrivest.data.repository.WalletRepository
...
class DashboardViewModel @Inject constructor(
        ...
        viewModelScope.launch {
            ...
            when (val result = profileRepository.getProfile()) {
                is Resource.Success -> {
                    ...
                    // Sync kyc_banner_hidden from server
                    result.data?.kycBannerHidden?.let { hidden ->
                        preferencesManager.kycBannerHidden = hidden
                    }
    ...
    fun getDepositWallet(): Wallet? {
        val preferred = preferencesManager.defaultCurrency ?: "UGX"
        return _wallets.value?.firstOrNull { it.currency == preferred }
            ?: _wallets.value?.firstOrNull { it.currency == "UGX" }
    ...
    fun formatBalance(balance: String, currency: String): String {
        if (_isAmountHidden.value == true) {
            return "•• ${preferencesManager.defaultCurrency ?: currency}"
        }

        val amount = balance.toDoubleOrNull() ?: 0.0
        val preferred = preferencesManager.defaultCurrency ?: currency

        val converted = co.afrivest.utils.CurrencyConverter.convert(amount, currency, preferred)
        return co.afrivest.utils.CurrencyConverter.format(converted, preferred)
    }
